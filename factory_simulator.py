"""
Factory Business Simulation Game - Enhanced Interactive Version (FIXED)
...
FIXES APPLIED:
- Added validation to prevent negative inventory (raw materials, WIP, finished goods)
- Added warnings for low cash situations
...
class FactorySimulator:
    ...
    def simulate_quarter(self,
        ...
        # Update inventory: receive materials and produce
        # Material: receive order, consume for production
        self.raw_material_inventory += material_purchase_lots

        # Warn if producing more than available materials (allows unrealistic scenarios)
        if production_lots > self.raw_material_inventory:
            print(f"⚠️  WARNING: Quarter {self.current_quarter} - Attempting to produce {production_lots} lots")
            print(f"   but only {self.raw_material_inventory} lots of raw materials available.")
            print(f"   This will result in negative inventory (unrealistic scenario).")
            print(f"   Consider purchasing {production_lots - self.raw_material_inventory} more lots of materials.\n")

        self.raw_material_inventory -= production_lots

        # WIP: produced - assembled
        # Check WIP capacity (though in current design it's immediately converted)
        if production_lots > self.work_in_progress:
            # WIP is immediately converted, but validate for future 2-stage implementation
            pass
        ...
        # Warn if cash is running low or negative (allows overdraft)
        if self.cash < 0:
            print(f"⚠️  WARNING: Quarter {self.current_quarter} - Cash is negative: {self.cash:.2f} M")
            print(f"   You may need to reduce spending or increase prices.\n")
        elif self.cash < 10.0:
            print(f"⚠️  WARNING: Quarter {self.current_quarter} - Cash is running low: {self.cash:.2f} M\n")
...
def run_interactive_game():
    ...
    for quarter in range(1, 5):
        ...
        try:
            ...
            print(f"❌ Fehler: {e}")
            print("Bitte passen Sie Ihre Entscheidungen an und versuchen Sie es erneut.")
            # Don't advance quarter, let user retry
            continue
